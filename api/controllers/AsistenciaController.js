...
module.exports = {
    ...
    updateAsisst: async function (req, res) {
      try {
        ...
        let hora = fecha[2].split(" ");
        let horaParseada = hora[1].split(":")
        let fechacompleta = new Date(hora[0],fecha[1]-1,fecha[0],horaParseada[0].trim()-3,horaParseada[1],horaParseada[2],00)
        let fechasalida =data.Asistencia.OutTime.split("/")
        let horasalida = fechasalida[2].split(" ");
        let horaParseadasalida = horasalida[1].split(":")
        let fechacompletasalida = new Date(horasalida[0],fechasalida[1]-1,fechasalida[0],horaParseadasalida[0].trim()-3,horaParseadasalida[1],horaParseadasalida[2],00)
        sails.log.info(fechacompleta);
        let datos = {
